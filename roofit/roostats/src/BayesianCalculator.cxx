...
struct  LikelihoodFunction {
   ...
   double operator() (const double *x ) const {
      ...
      if (nCalls > 0 && nCalls % 1000 == 0) {
         ooccoutD((TObject*)nullptr,Eval) << "Likelihood evaluation ncalls = " << nCalls
                                    << " x0 " << x[0] << "  nll = " << nll+fOffset;
         if (fPrior) ooccoutD((TObject*)nullptr,Eval) << " prior(x) = " << (*fPrior)(x);
         ooccoutD((TObject*)nullptr,Eval) << " likelihood " << likelihood
      ...
      if  (likelihood > fMaxL ) {
         fMaxL = likelihood;
         if ( likelihood > 1.E10) {
            ooccoutW((TObject*)nullptr,Eval) << "LikelihoodFunction::()  WARNING - Huge likelihood value found for  parameters ";
            for (int i = 0; i < fFunc.nObs(); ++i)
               ooccoutW((TObject*)nullptr,Eval) << " x[" << i << " ] = " << x[i];
            ooccoutW((TObject*)nullptr,Eval) << "  nll = " << nll << " L = " << likelihood << std::endl;
         }
      }
...
public:
      ...
      fHasNorm(false),  fUseOldValues(true), fError(false)
   {
      ...
      ooccoutD((TObject*)nullptr,NumIntegration) << "PosteriorCdfFunction::Compute integral of posterior in nuisance and poi. "
      ...
      for (unsigned int i = 0; i < fXmin.size(); ++i) {
         ...
         fXmax[i] = var.getMax();
         par[i] = var.getVal();
         ooccoutD((TObject*)nullptr,NumIntegration) << "PosteriorFunction::Integrate" << var.GetName()
                                              << " in interval [ " <<  fXmin[i] << " , " << fXmax[i] << " ] " << std::endl;
      }

      fIntegrator.Options().Print(ooccoutD((TObject*)nullptr,NumIntegration));
      ...
      fNorm = (*this)( fMaxPOI );
      if (fError) ooccoutE((TObject*)nullptr,NumIntegration) << "PosteriorFunction::Error computing normalization - norm = " << fNorm << std::endl;
      fHasNorm = true;
   ...
   ROOT::Math::IGenFunction * Clone() const {
      ooccoutD((TObject*)nullptr,NumIntegration) << " cloning function .........." << std::endl;
...
private:
   ...
   double DoEval (double x) const {
      ...
      ooccoutD((TObject*)nullptr,NumIntegration) << "PosteriorCdfFunction: poi = [" << fXmin[0] << " , "
      ...
      if (TMath::IsNaN(cdf) || cdf > std::numeric_limits<double>::max()) {
         ooccoutE((TObject*)nullptr,NumIntegration) << "PosteriorFunction::Error computing integral - cdf = "
                                              << cdf << std::endl;
         fError = true;
      }

      if (cdf != 0 && error/cdf > 0.2 )
         oocoutW((TObject*)nullptr,NumIntegration) << "PosteriorCdfFunction: integration error  is larger than 20 %   x0 = " << fXmin[0]
                                              << " x = " << x << " cdf(x) = " << cdf << " +/- " << error << std::endl;

      if (!fHasNorm) {
         oocoutI((TObject*)nullptr,NumIntegration) << "PosteriorCdfFunction - integral of posterior = "
                                             << cdf << " +/- " << error << std::endl;
         fNormErr = error;
         return cdf;
      }
      ...
      if (normcdf > 1. + 3 * errnorm) {
         oocoutW((TObject*)nullptr,NumIntegration) << "PosteriorCdfFunction: normalized cdf values is larger than 1"
...
public:
      ...
      fError(0)
   {
      ...
      ooccoutD((TObject*)nullptr,NumIntegration) << "PosteriorFunction::Evaluate the posterior function by integrating the nuisances: " << std::endl;
      for (unsigned int i = 0; i < fXmin.size(); ++i) {
      ...
      if (fXmin.size() == 1) { // 1D case
         ...
         fIntegratorOneDim->Options().Print(ooccoutD((TObject*)nullptr,NumIntegration) );
      }
         ...
         // print the options
         opt.Print(ooccoutD((TObject*)nullptr,NumIntegration) );
      }
   }
...
private:
   double DoEval (double x) const {
      ...
      // debug
      ooccoutD((TObject*)nullptr,NumIntegration) << "PosteriorFunction:  POI value  =  "
                                           << x << "\tf(x) =  " << f << " +/- " << error
                                           << "  norm-f(x) = " << f/fNorm
                                           << " ncalls = " << fFunctor.binding().numCall() << std::endl;
      ...
      if (f != 0 && error/f > 0.2 )
         ooccoutW((TObject*)nullptr,NumIntegration) << "PosteriorFunction::DoEval - Error from integration in "
...
public:
      ...
      fRedoToys(redoToys)
   {
      ...
      ooccoutI((TObject*)nullptr,InputArguments) << "PosteriorFunctionFromToyMC::Evaluate the posterior function by randomizing the nuisances:  niter " << fNumIterations << std::endl;

      ooccoutI((TObject*)nullptr,InputArguments) << "PosteriorFunctionFromToyMC::Pdf used for randomizing the nuisance is " << fPdf->GetName() << std::endl;
      ...
      for (int i = 0; i < fNuisParams.getSize(); ++i) {
         if (!vars->find( fNuisParams[i].GetName() ) ) {
            ooccoutW((TObject*)nullptr,InputArguments) << "Nuisance parameter " << fNuisParams[i].GetName()
      ...
      if (!fRedoToys) {
         ooccoutI((TObject*)nullptr,InputArguments) << "PosteriorFunctionFromToyMC::Generate nuisance toys only one time (for all POI points)" << std::endl;
         GenerateToys();
      }
   }
   ...
   void GenerateToys() const {
      ...
      if(fGenParams==nullptr) {
         ooccoutE((TObject*)nullptr,InputArguments) << "PosteriorFunctionFromToyMC - failed to generate nuisance parameters" << std::endl;
      }
   }
   ...
   double DoEval( double x) const {
      ...
      for(int iter=0; iter<fNumIterations; ++iter) {
         ...
         if( fval > std::numeric_limits<double>::max()  ) {
            ooccoutE((TObject*)nullptr,Eval) <<  "BayesianCalculator::EvalPosteriorFunctionFromToy : "
                        << "Likelihood evaluates to infinity " << std::endl;
            ooccoutE((TObject*)nullptr,Eval) <<  "poi value =  " << x << std::endl;
            ooccoutE((TObject*)nullptr,Eval) <<  "Nuisance  parameter values :  ";
            for (int i = 0; i < npar; ++i)
               ooccoutE((TObject*)nullptr,Eval) << fNuisParams[i].GetName() << " = " << p[i] << " ";
            ooccoutE((TObject*)nullptr,Eval) <<  " - return 0   " << std::endl;
      ...
      // debug
      ooccoutD((TObject*)nullptr,NumIntegration) << "PosteriorFunctionFromToyMC:  POI value  =  "
                                           << x << "\tp(x) =  " << val << " +/- " << fError << std::endl;


      if (val != 0 && fError/val > 0.2 ) {
         ooccoutW((TObject*)nullptr,NumIntegration) << "PosteriorFunctionFromToyMC::DoEval"
...
BayesianCalculator::BayesianCalculator() :
   ...
   fProductPdf (nullptr), fLogLike(nullptr), fLikelihood (nullptr), fIntegratedLikelihood (nullptr), fPosteriorPdf(nullptr),
